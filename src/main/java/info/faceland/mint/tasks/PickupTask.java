...
public class PickupTask extends BukkitRunnable {
  ...
  public PickupTask(Item sourceItem) {
    ...
    runTaskTimer(MintPlugin.getInstance(), (int) (15 * Math.random()), 6L);
  }
  ...
  public void run() {
    ...
    List<Player> players = new ArrayList<>(sourceItem.getWorld().getNearbyPlayers(sourceItem.getLocation(), 1.5));
    ...
    if (players.size() > 1) {
      players = players.stream().filter(e -> !e.isDead()).sorted(distanceComparator).collect(Collectors.toList());
    }
